...
const formatChatTitle = (rawText) => {
  ...
  const cleaned = rawText
    .trim()
    .replace(/^(what is|what are|how can i|how to|where is|tell me about|is there a|can you explain)\s+/i, "")
    .replace(/[?.!]+$/, "");

  const capitalized = cleaned.charAt(0).toUpperCase() + cleaned.slice(1);
  return capitalized.length > 26 ? capitalized.slice(0, 26).trim() + "..." : capitalized;
};
...
export default function AiAssistant() {
  ...
  const handleSendMessage = async (textToSend) => {
    ...
    try {
      ...
      const topicTitle = formatChatTitle(text);

      if (!conversationId) {
        const conversation = await createConversation({ title: topicTitle });
        conversationId = conversation?.id || conversation?._id || Date.now().toString();
        setActiveConversationId(conversationId);
